...
export default function LoginGastronomico() {
  ...
  const syncUserToDb = async (user: any, extraData: any = {}) => {
    ...
    if (!userDoc.exists()) {
      await setDoc(userDocRef, {
         ...
         role: resolvedRole,
         plan: "pending", // Plan will be determined in Onboarding
         onboardingCompleted: isOboardingComplete, 
      ...
      isOboardingComplete = data.onboardingCompleted === true;

      // Force admin role for this specific email if not set
      if (user.email === "admin@digitalbite.app" && resolvedRole !== "admin") {
         await updateDoc(userDocRef, { role: "admin", onboardingCompleted: true });
         resolvedRole = "admin";
         isOboardingComplete = true;
      }
    }
    return { role: resolvedRole, isOboardingComplete };
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (email === "admin@digitalbite.app" && password === "123") {
      ...
      if (isRegister) {
        ...
        const { role, isOboardingComplete } = await syncUserToDb(userCredential.user);
        if (role === "admin") {
          router.push("/admin/plantillas");
        } else if (!isOboardingComplete) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const { role, isOboardingComplete } = await syncUserToDb(result.user);
      if (role === "admin") {
        router.push("/admin/plantillas");
      } else if (!isOboardingComplete) {
        router.push("/onboarding");
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
           ...
           <h1 className="text-3xl font-black text-slate-900 tracking-tight">DigitalBite App</h1>
           <p className="text-slate-500 mt-2 text-sm">{isRegister ? "Crea una cuenta para empezar" : "Inicia sesión para crear tus menús y diseños."}</p>
        </div>
